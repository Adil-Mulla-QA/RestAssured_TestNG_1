...
public class Get_TC3 {
	 ...
	 public static void TestSetup() {
		 ...
		 log_dir = Directory.Create_Log_Directory("Get_TC3");
	...
	public static void Validator(String ResponseBody) {
		...
		String res_Id = jspRes.getString("data[3].id");
		String res_Email  = jspRes.getString("data[3].email");
		String res_FirstName = jspRes.getString("data[3].first_name");
		String res_LastName  = jspRes.getString("data[3].last_name");
		String res_Avatar  = jspRes.getString("data[3].avatar");
		// Validate ResponseBody by Using TestNG
		
		Assert.assertNotNull(res_Id);
		
		Assert.assertEquals(res_Id, "10");
		Assert.assertEquals(res_Email, "byron.fields@reqres.in");
		Assert.assertEquals(res_FirstName, "Byron");		
		Assert.assertEquals(res_LastName, "Fields");
		Assert.assertNotNull(res_Avatar);
	}
	
	@AfterTest
	public static void Test_TearDown() throws IOException {
		String TestClassName = Get_TC3.class.getName();
