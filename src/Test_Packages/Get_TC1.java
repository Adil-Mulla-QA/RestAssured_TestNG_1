...
public class Get_TC1 {
	...
	public static void Validator(String ResponseBody) {
		...
		String res_Id = jspRes.getString("data[0].id");
		String res_Email  = jspRes.getString("data[0].email");
		String res_FirstName = jspRes.getString("data[0].first_name");
		String res_LastName  = jspRes.getString("data[0].last_name");
		String res_Avatar  = jspRes.getString("data[0].avatar");
		...
		Assert.assertEquals(res_FirstName, "Michael");		
		Assert.assertEquals(res_LastName, "Lawson");
		Assert.assertNotNull(res_Avatar);
	}
	
	@AfterTest
	public static void Test_TearDown() throws IOException {
		String TestClassName = Get_TC1.class.getName();
